...
\newcommand{\EinflussDesListentypsAnalyse}{
    Die gemesen daten zeigen deutlch das das der becase von qiksort in der prxis beser ist als \(n \log_2(n)\). Das ligt vermutlich daren das ich in der relise verion (mit compier optimirung) messe.
    Da
    In der Debug-Version (ohne Compiler-Optimierung) habe ich bei einem sortierten Array der Größe 400 Mio. eine Laufzeit von 14~s bei Quicksort gemessen, welche wiederum größer als \(n \log_2(n)\) ist.

    Die Messungen zeigen, dass invertiert sortierte Arrays und sortierte Arrays nahezu identische Laufzeiten erzeugen. Die entsprechenden Kurven würden fast aufeinanderliegen, weshalb ich die grafische Darstellung der invertiert sortierten Arrays zur Übersicht nicht mit aufgenommen habe.

    Dasselbe gilt für fastsortierte Arrays, weshalb ich diese ebenfalls nicht gesondert grafisch darstelle.
}