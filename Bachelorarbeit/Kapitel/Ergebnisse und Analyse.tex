...
\newcommand{\TiefenbasierteThreadErzeugungAnalyse}{
    ...
    Dies ist auch deutlich im Strong Scaling (Abbildung~\ref{fig:TiefenbasierteThreadErzeugungDiagrammB}) zu erkennen. Dort zeigt sich, dass Quicksort bei dieser Liste weiterhin an Performance gewinnt, obwohl die Threadanzahl die Anzahl der logischen Prozessoren übersteigt. Während bei Mergesort zu beobachten ist, dass die gemessene Laufzeitverbesserung nur geringfügig schlechter ausfällt als die theoretisch zu erwartende.
    \newline
    Beim Weak Scaling (Abbildung~\ref{fig:TiefenbasierteThreadErzeugungDiagrammC}) ist zu erkennen, dass Quicksort dort sehr schlecht abschneidet. Dies liegt ebenfalls an der unausgeglichenen Thread-Auslastung sowie daran, dass nur eine zufällige Liste gemessen wurde und kein Median über mehrere Listen verwendet wird.
    \newline
...
\newcommand{\WorkerthreadsErwartung}{
    ...
    Dazu wird für die Mergesort-Worker-Thread-Variante erwartet, dass es zu einer Laufzeitverbesserung kommt, bis 32 Threads erzeugt werden.
    Dies liegt an dem Wait und wird nur durch \texttt{addTaskSmart} kompensiert (Listing \ref{lst:mergesortW}), indem die Funktion dafür sorgt, dass sich immer mindestens die Hälfte aller Threads nicht gegenseitig blockieren.
    \newline
...
\newcommand{\WorkerthreadsAnalyse}{
    ...
    Wenn man jetzt die Quicksort-Worker-Thread-Variante mit der Mergesort-tiefenbasierten Thread-Erzeugungsvariante vergleicht, stellt man immer noch fest, dass Quicksort im Durchschnitt doppelt so schnell ist wie Mergesort. Beim Strong Scaling (Abbildung~\ref{fig:WorkerthreadsDiagrammB}) ist außerdem zu beobachten, dass die Performance bei sehr vielen Threads wieder abnimmt. Ursache hierfür sind die Sperren zur Thread-Arbeitsverteilung, die in dieser Variante erforderlich für die gemeinsame Aufgaben-Queue sind. Bei sehr vielen Threads führt dies zu einem Performance-Einbruch, da jeweils nur ein Thread die Sperre belegen kann. Hinzu kommen die Kosten der reinen Initialisierungszeit, da $2^{14}$ Threads rund 1~s für ihre Erstellung benötigen.
...
\newcommand{\EinflussDesDatentypsAnalyse}{
    ...
    \newline
    Ganz besonders fällt auf, dass Mergesort beim Strong Scaling (Abbildung~\ref{fig:EinflussDesDatentypsDiagrammB}) schon nach 8 Threads nur noch eine minimale Performanceverbesserung erzielt.
    Dies liegt an dem genannten Kopieren, welches wahrscheinlich dazu führt, dass der Cache vollläuft und somit Cache-Misses entstehen.
    Diese sorgen dann wieder für erhöhte Speicherlatenzen und verursachen somit auch die nur geringe Performanceverbesserung.
    \newline
