...
\newcommand{\MessumgebungUndHardware}{
    ...
    \newline
    Als Betriebssystem kam Windows~10 in der Version~22H2 zum Einsatz.
    Der Code wurde in \texttt{C++20} implementiert und mittels CMake (Version 3.26.4) sowie dem MSVC-Compiler (Version 14.44.35207) unter Verwendung des \texttt{MultiThreaded}-Flags kompiliert.
    Die Ausführung der Messungen erfolgte im integrierten Terminal von Visual Studio Code in einer Release-Konfiguration. Abweichungen hiervon werden an entsprechender Stelle gesondert angegeben.
}
...
\newcommand{\Implementierungsvarianten}{
    ...
    Da die Rekursionstiefe eines balancierten Binärbaums auf $\log_2(n)$ Ebenen begrenzt ist, ergibt sich $e_{\max} = \log_2(n)$.
    Weil ebenenweise parallelisiert wird, gilt $e = \log_2(p)$, was schließlich zu $p_{\max} = n$ führt.
    ...
    \begin{flalign*}
        ...
        O(T(n)) & = O(n^2).                        &
    \end{flalign*}
    Für die Work-Stealing-Variante ist auch keine exakte Vorhersage der Laufzeit möglich, weshalb eine Abschätzung erfolgt.
...
% \section{Optimirung}
\newcommand{\Optimierung}{
    Auf eine Optimierung des Codes wurde bewusst verzichtet, da es vorrangig um den Vergleich der Threading-Strategien geht und eine perfekte Implementierung zu zeitaufwendig gewesen wäre.
    Da das Rausoptimieren des Waits der (Worker-Thread-Variante Mergesort, \ref{lst:mergesortW}) Variante viel zu aufwendig wäre und auch dann kein einfaches Beispiel mehr ist.
    Daher wurde gleich vollständig auf eine Optimierung verzichtet, da ich damit nicht alle Varianten gleichmäßig mehr optimieren konnte.
}