...
public:
    std::function<void(int *, int, int, MergeWorkerPool &)> taskHandler;

public:
    MergeWorkerPool(int numThreads) : activeTasks(0), finished(false) {
        for (int i = 0; i < numThreads; ++i) {
            threads.emplace_back(&MergeWorkerPool::worker, this);
        }
    }
